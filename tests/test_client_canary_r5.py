...
def test_client_canary_r5_end_to_end_flow() -> None:
    """R5 Proof: End-to-end Client/BFF -> Admission -> Job Ledger -> Dispatch -> Callback -> Audit flow."""
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        secret = "r5-canary-secret-key-98765"

        # 1. Client/BFF -> Admission Policy Check
        decision = policy_store.evaluate_admission(client_id="flowbiz-client-dhamma")
        assert decision.allowed is True

        # 2. Platform Job Ledger creation
        ...
        # 4. Issue callback token & mark SENT
        ...
        dispatch_store.finalize_dispatch(
            ...
            response_code=202,
            error=None,
            sent_at="2026-08-10T21:30:00Z",
        )

        # 5. Verified Callback
        is_token_valid = dispatch_store.verify_callback_token(
            dispatch_id=dispatch.dispatch_id,
            provided_token=raw_token,
        )
        assert is_token_valid is True

        _, updated_dispatch = dispatch_store.apply_callback(
            dispatch_id=dispatch.dispatch_id,
            job_id=job.job_id,
            callback_status=CALLBACK_STATUS_SUCCESS,
            callback_occurred_at="2026-08-10T21:30:05Z",
            callback_received_at="2026-08-10T21:30:06Z",
        )
        assert updated_dispatch.callback_status == CALLBACK_STATUS_SUCCESS

        # 6. Audit / Observability Event Recording
        audit_event = RequestEvent(
            route="/v1/platform/canary/dhamma",
            status_code=200,
            duration_ms=18.4,
        )
        assert audit_event.route == "/v1/platform/canary/dhamma"
