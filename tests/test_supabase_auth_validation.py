...
def test_invalid_signature_is_rejected(signing_material) -> None:
    ...
    with pytest.raises(SupabaseAuthError, match="Invalid bearer token"):
        validator_for(jwks).validate(mint_token(other_key))
...
def test_expired_token_is_rejected(signing_material) -> None:
    ...
    with pytest.raises(SupabaseAuthError, match="Invalid bearer token"):
        validator_for(jwks).validate(
            mint_token(private_key, exp=int(time.time()) - 120)
        )
...
def test_wrong_issuer_and_audience_are_rejected(signing_material) -> None:
    ...
    with pytest.raises(SupabaseAuthError, match="Invalid bearer token"):
        validator_for(jwks).validate(
            mint_token(private_key, iss="https://wrong.example.test")
        )
    with pytest.raises(SupabaseAuthError, match="Invalid bearer token"):
...
def test_jwks_fetch_failure_fails_closed(signing_material) -> None:
    private_key, _jwks = signing_material
...
def test_dependency_accepts_valid_token_and_does_not_authorize_tenant(
    signing_material,
) -> None:
    private_key, jwks = signing_material
